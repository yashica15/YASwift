...
class YARealmTVC: UITableViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        arrPerson = YARLMManager().fetchPersonList()
    ...
    func addEditPerson(isEdit:Bool, objPerson:Person?) -> Void {
        ...
        saveAction = UIAlertAction(title: "Save", style: .default, handler: {
            ...
            if isEdit {
                let isResult = YARLMManager().updatePersonObject(objPerson: objPerson!, name: (firstTextField.text?.trimmedString())!, birthDate: self.YADatePicker.date as NSDate)
                ...
                if isResult == false {
                    ...
                    self.arrPerson = YARLMManager().fetchPersonList()
                ...
                let isResult = YARLMManager().savePersonObject(objPerson: objPerson)
                if isResult == false {
                    YAAppDelegate.getDelegate().toastView(hudType: .YAShowToastMessage, strToastMessage: "There is some error in add. Please try later.")
                } else {
                    self.arrPerson = YARLMManager().fetchPersonList()
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let isResult = YARLMManager().deletePersonObject(objPerson: (self.arrPerson?[indexPath.row])!)
